 ...
 * 
 * add ko.mapping by longchan @ 2018.03.24
 */
...
(function () {
	...
	function extendObject(destination, source) {
		...
		for (var key in source) {
			if (source.hasOwnProperty(key) && source[key]) {
				destType = ko.mapping.getType(destination[key]);
				if (key && destination[key] && destType !== "array" && destType !== "string") {
					extendObject(destination[key], source[key]);
				} else {
					var bothArrays = ko.mapping.getType(destination[key]) === "array" && ko.mapping.getType(source[key]) === "array";
					if (bothArrays) {
	...
	ko.mapping.toJS = function (rootObject, options) {
		...
		if (ko.mapping.getType(defaultOptions.ignore) !== "array") throw new Error("ko.mapping.defaultOptions().ignore should be an array.");
		if (ko.mapping.getType(defaultOptions.include) !== "array") throw new Error("ko.mapping.defaultOptions().include should be an array.");
		if (ko.mapping.getType(defaultOptions.copy) !== "array") throw new Error("ko.mapping.defaultOptions().copy should be an array.");
	...
	function mergeArrays(a, b) {
		if (ko.mapping.getType(a) !== "array") {
			if (ko.mapping.getType(a) === "undefined") a = [];
			else a = [a];
		}
		if (ko.mapping.getType(b) !== "array") {
	...
	function updateViewModel(mappedRootObject, rootObject, options, parentName, parent, parentPropertyName, mappedParent) {
		var isArray = ko.mapping.getType(ko.utils.unwrapObservable(rootObject)) === "array";
	...
	function visitPropertiesOrArrayEntries(rootObject, visitorCallback) {
		if (ko.mapping.getType(rootObject) === "array") {
			for (var i = 0; i < rootObject.length; i++)
			visitorCallback(i);
		} else {
			for (var propertyName in rootObject)
	...
	function getPropertyName(parentName, parent, indexer) {
		...
		if (ko.mapping.getType(parent) === "array") {
			if (parentName) {
	...
	ko.mapping.visitModel = function (rootObject, callback, options) {
		...
		if (!canHaveProperties(unwrappedRootObject)) {
			...
			mappedRootObject = ko.mapping.getType(unwrappedRootObject) === "array" ? [] : {};
		}
			...
			if (ko.utils.arrayIndexOf(options.copy, indexer) === -1) {
				...
				if (ko.utils.arrayIndexOf(options.include, indexer) === -1) {
				    ...
				    if (unwrappedRootObject[mappingProperty]
				        ...
				        && !(ko.mapping.getType(unwrappedRootObject) === "array")) {
						return;
					}
				}
			}
